...
class Program {
  ...
  function __construct($program_string) {
    $this->name = explode(' ', $program_string)[0];
    $this->weight = preg_match('/\((\d+)\)/', $program_string, $weight) ? $weight[1] : NULL;
    $children_str = '';
    if (preg_match('/.* \(\d+\) -> (.*)/', $prog_str, $children_str)) {
      $parents[] = $name;
      $this->children = array_map('trim', explode(',', $children_str[1]));
    }
  }
}
...
$parents = [];
foreach (explode("\n", $input) as $prog_str) {
  $name = explode(' ', $prog_str)[0];
  $list[$name] = [
    'weight' => preg_match('/\((\d+)\)/', $prog_str, $weight) ? $weight[1] : NULL,
    'children' => [],
  ];
  $children_str = '';
  if (preg_match('/.* \(\d+\) -> (.*)/', $prog_str, $children_str)) {
    $parents[] = $name;
    $list[$name]['children'] = array_map('trim', explode(',', $children_str[1]));
  }
}
foreach ($parents as $parent) {
  $is_child = FALSE;
  foreach ($parents as $program) {
    if (in_array($parent, $list[$program]['children'])) {
      $is_child = TRUE;
    }
  }
  if (!$is_child) {
...
function total_weight($program_name) {
  global $list;
  return $list[$program_name]['weight'] + array_reduce(array_map('total_weight', $list[$program_name]['children']), 'sum');
}

// Leaf nodes of a single parent must all be alike.
// Children must be
// Descend to leaf nodes and work way back up to first unbalanced.
function find_unbalanced_children($parent) {
  global $list;
  $list[$parent]['total_weight'] = total_weight($parent);
  if (!empty($list[$parent]['children'])) {
    $uniq_weights = [];
    foreach ($list[$parent]['children'] as $child) {
      $list[$child]['total_weight'] = total_weight($child);
      $uniq_weights[$list[$child]['total_weight']][] = $child;
    }
    foreach ($uniq_weights as $weight => $names) {
      if (count($names) == 1) {
        foreach ($uniq_weights as $w => $n) {
          if ($w == $weight) {
            continue;
          }
          $diff = $weight > $w ? $w - $weight : $weight - $w;
          $target_val = $list[$names[0]]['weight'] + $diff;
          print $names[0] . " should be $target_val\n";
        }
        find_unbalanced_children($names[0]);
      }
    }
  }
}
