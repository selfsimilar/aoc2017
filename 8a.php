...
foreach (explode("\n", $input) as $inst) {
  ...
  if (!array_key_exists($cond_target, $registers)) {
    $registers[$cond_target] = 0;
  }
  $pass = FALSE;
  switch ($cond_op) {
    case '>' :
      if ($registers[$cond_target] > $cond_val) $pass = TRUE;
      break;
    case '<' :
      if ($registers[$cond_target] < $cond_val) $pass = TRUE;
      break;
  ...
  if ($registers[$target] > $largest_ever) {
    $largest_ever = $registers[$target];
  }
}
